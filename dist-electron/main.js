...
const _HotkeyManager = class _HotkeyManager {
  constructor() {
    __publicField(this, "registeredHotkeys", /* @__PURE__ */ new Map());
    __publicField(this, "config", null);
    __publicField(this, "configPath");
    __publicField(this, "mainWindow", null);
    // Slot ID mapping for number keys 1-9
    __publicField(this, "SLOT_KEYS", ["1", "2", "3", "4", "5", "6", "7", "8", "9"]);
    this.configPath = path__namespace.join(
  ...
  saveConfig() {
    try {
      ...
      fs__namespace.writeFileSync(this.configPath, JSON.stringify(this.config, null, 2), "utf-8");
    } catch (error) {
  ...
  registerAllHotkeys() {
    ...
    for (let i = 1; i <= 9; i++) {
      ...
      const accelerator = ((_a = this.config) == null ? void 0 : _a.hotkeys[key]) || String(i);
      this.registerHotkey(accelerator, () => this.handleSwitchScript(i));
    }
    const upKey = ((_b = this.config) == null ? void 0 : _b.hotkeys.preview_up) || "Up";
    const downKey = ((_c = this.config) == null ? void 0 : _c.hotkeys.preview_down) || "Down";
    this.registerHotkey(upKey, () => this.handlePreviewScript("up"));
    this.registerHotkey(downKey, () => this.handlePreviewScript("down"));
    const spaceKey = ((_d = this.config) == null ? void 0 : _d.hotkeys.toggle_pause) || "Space";
    this.registerHotkey(spaceKey, () => this.handleTogglePause());
    const helpKey = ((_e = this.config) == null ? void 0 : _e.hotkeys.show_help) || "?";
    this.registerHotkey(helpKey, () => this.handleShowHelp());
    const escKey = ((_f = this.config) == null ? void 0 : _f.hotkeys.hide_help) || "Escape";
  ...
  registerHotkey(accelerator, callback) {
    ...
    try {
      const success = electron.globalShortcut.register(accelerator, callback);
      if (success) {
   ...
   */
  updateHotkey(action, newAccelerator) {
...
function registerHotkeyHandlers() {
  ...
  electron.ipcMain.handle("layout:save", async (_event, layout) => {
    const fs2 = await import("fs");
    const path2 = await import("path");
    const configDir = path2.join(process.cwd(), "data", "config");
    if (!fs2.existsSync(configDir)) {
      fs2.mkdirSync(configDir, { recursive: true });
    }
    fs2.writeFileSync(path2.join(configDir, "layout.json"), JSON.stringify(layout, null, 2));
    return true;
  });
  electron.ipcMain.handle("layout:load", async () => {
    const fs2 = await import("fs");
    const path2 = await import("path");
    const configPath = path2.join(process.cwd(), "data", "config", "layout.json");
...
function getDbPath() {
  ...
  return path$1.join(userDataPath, "data", "wordshot.db");
}
...
function getScriptById(id) {
  ...
  return row ? parseScript(row) : void 0;
}
...
function getSegmentsByTemplate(templateId) {
  const database = getDatabase();
...
function registerDatabaseHandlers() {
  ...
  electron.ipcMain.handle("db:getAllScripts", () => getAllScripts());
  electron.ipcMain.handle("db:getScriptById", (_, id) => getScriptById(id));
  electron.ipcMain.handle("db:createScript", (_, script) => createScript(script));
  electron.ipcMain.handle("db:updateScript", (_, id, updates) => updateScript(id, updates));
  electron.ipcMain.handle("db:deleteScript", (_, id) => deleteScript(id));
  electron.ipcMain.handle("db:searchScripts", (_, query) => searchScripts(query));
  electron.ipcMain.handle("db:getAllTemplates", () => getAllTemplates());
  electron.ipcMain.handle("db:getTemplateById", (_, id) => getTemplateById(id));
  electron.ipcMain.handle("db:createTemplate", (_, template) => createTemplate(template));
  electron.ipcMain.handle("db:updateTemplate", (_, id, updates) => updateTemplate(id, updates));
  electron.ipcMain.handle("db:deleteTemplate", (_, id) => {
    console.log("[IPC] db:deleteTemplate called with id:", id);
    return deleteTemplate(id);
  });
  electron.ipcMain.handle("db:getSegmentsByTemplate", (_, templateId) => getSegmentsByTemplate(templateId));
  electron.ipcMain.handle("db:createSegment", (_, segment) => createSegment(segment));
  electron.ipcMain.handle("db:updateSegment", (_, id, updates) => updateSegment(id, updates));
  electron.ipcMain.handle("db:deleteSegment", (_, id) => deleteSegment(id));
  electron.ipcMain.handle("ai:testConnection", async (_, provider) => {
    ...
    try {
      ...
      if (errorMsg.includes("aborted")) {
        return { success: false, message: `连接超时 (${provider.timeout / 1e3}s)` };
      }
      return { success: false, message: `连接失败: ${errorMsg}` };
    }
  });
  electron.ipcMain.handle("db:clearAllScripts", () => {
    console.log("[IPC] Clearing all scripts from database");
    return clearAllScripts();
  });
  console.log("[IPC] ai:testConnection handler registered");
  electron.ipcMain.handle("ai:chatCompletion", async (_, request) => {
    var _a;
    console.log("[IPC] ai:chatCompletion called");
    console.log("[IPC] Provider:", JSON.stringify({ ...request.provider, apiKey: request.provider.apiKey ? "***" + request.provider.apiKey.slice(-4) : void 0 }));
    const { provider, messages, temperature = 0.7, max_tokens = 128e3 } = request;
    const effectiveTimeout = Math.max(provider.timeout, 6e5);
    const headers = {
      "Content-Type": "application/json",
    ...
    try {
      ...
      if (data.choices && data.choices[0]) {
        return { success: true, content: ((_a = data.choices[0].message) == null ? void 0 : _a.content) || "", usage: data.usage };
      }
      if (data.text || data.result) {
        return { success: true, content: data.text || data.result, usage: data.usage };
      }
      if (typeof data === "string") {
        return { success: true, content: data, usage: void 0 };
      }
      ...
      if (errorMsg.includes("aborted")) {
        return { success: false, error: `连接超时 (${effectiveTimeout / 1e3}s)` };
      }
...
var spawnWorker$2 = ({ workerPath }) => new Worker(workerPath);
...
const fs = fs$1;
const fetch$1 = commonjsGlobal.fetch || require$$2;
const isURL = isUrl_1;
...
var loadImage$2 = async (image) => {
  ...
  if (typeof image === "string") {
    if (isURL(image) || image.startsWith("moz-extension://") || image.startsWith("chrome-extension://") || image.startsWith("file://")) {
      const resp = await fetch$1(image);
...
const Tesseract = /* @__PURE__ */ getDefaultExportFromCjs(src);
let worker = null;
...
function parseOCRDanmu(rawText) {
  const results = [];
...
function classifyDanmuType(content, username) {
  const text = content.toLowerCase();
  username.toLowerCase();
  if (text.includes("火箭") || text.includes("飞船") || text.includes("超级火箭") || /\d{3,}[元¥]/.test(text)) {
    return "big_gift";
  }
  if (text.includes("礼物") || text.includes("打赏") || text.includes("送") || text.includes("赞")) {
    return "gift";
  }
  if (text.includes("关注") || text.includes("follow") || text.includes("粉丝")) {
    return "follower";
  }
  if (text.includes("?") || text.includes("？") || text.includes("怎么") || text.includes("如何") || text.includes("为什么")) {
    return "question";
  }
  if (text.includes("滚") || text.includes("垃圾") || text.includes("恶心") || text.includes("差评")) {
    return "hater";
  }
  if (text.includes("不服") || text.includes("来啊") || text.includes("挑战")) {
    return "provocative";
  }
  if (text.includes("贵宾") || text.includes("VIP") || text.includes("老爷")) {
    return "vip";
  }
  if (text.includes("好看") || text.includes("漂亮") || text.includes("棒") || text.includes("赞") || text.includes("厉害")) {
    return "praise";
  }
  if (text.includes("PK") || text.includes("pk") || text.includes("挑战")) {
    return "pk";
  }
  return "normal";
}
function isValidDanmuText(text) {
  if (text.length < 2 || text.length > 500) return false;
  const letterCount = text.replace(/[^a-zA-Z一-龥]/g, "").length;
  if (letterCount < 2) return false;
  return true;
}
const DANMU_CHANNELS = {
...
const CONFIG_DIR = path__namespace.join(process.cwd(), "data", "config");
const CONFIG_FILE = path__namespace.join(CONFIG_DIR, "danmu_capture.json");
...
const _DanmuCaptureService = class _DanmuCaptureService {
  constructor() {
    __publicField(this, "config");
    __publicField(this, "selectedWindow", null);
    __publicField(this, "captureTimer", null);
    __publicField(this, "isCapturing", false);
    __publicField(this, "isPaused", false);
    __publicField(this, "ocrInitialized", false);
    // Deduplication: time window in ms (default 30 minutes)
    __publicField(this, "dedupTimeWindowMs", 30 * 60 * 1e3);
    // Deduplication: content similarity threshold (0-1)
    __publicField(this, "dedupSimilarityThreshold", 0.85);
    __publicField(this, "dedupState", {
      seenDanmu: /* @__PURE__ */ new Map(),
      recentContents: []
    });
    __publicField(this, "mainWindow", null);
  ...
  static getInstance() {
    if (!_DanmuCaptureService.instance) {
      _DanmuCaptureService.instance = new _DanmuCaptureService();
    }
    return _DanmuCaptureService.instance;
  }
  /**
   ...
   */
  registerIpcHandlers() {
    electron.ipcMain.handle(DANMU_CHANNELS.GET_WINDOWS, async () => {
      return this.getAvailableWindows();
    });
    electron.ipcMain.handle(DANMU_CHANNELS.SELECT_WINDOW, async (_event, windowId) => {
      return this.selectWindow(windowId);
    });
    electron.ipcMain.handle(DANMU_CHANNELS.UPDATE_CONFIG, async (_event, config) => {
      return this.updateConfig(config);
    });
    electron.ipcMain.handle(DANMU_CHANNELS.GET_CONFIG, async () => {
      return this.config;
    });
    electron.ipcMain.handle(DANMU_CHANNELS.CAPTURE_START, async () => {
      return this.startCapture();
    });
    electron.ipcMain.handle(DANMU_CHANNELS.CAPTURE_STOP, async () => {
      return this.stopCapture();
    });
    electron.ipcMain.handle(DANMU_CHANNELS.CAPTURE_PAUSE, async () => {
      return this.pauseCapture();
    });
    electron.ipcMain.handle(DANMU_CHANNELS.CAPTURE_RESUME, async () => {
  ...
  loadConfig() {
    ...
    try {
      if (fs__namespace.existsSync(CONFIG_FILE)) {
        const content = fs__namespace.readFileSync(CONFIG_FILE, "utf-8");
  ...
  saveConfig() {
    ensureConfigDir();
    try {
      fs__namespace.writeFileSync(CONFIG_FILE, JSON.stringify({ config: this.config }, null, 2), "utf-8");
    ...
    try {
      ...
      const windows = sources.map((source) => {
        ...
        return {
          ...
          selected: ((_a = this.selectedWindow) == null ? void 0 : _a.id) === source.id
        };
      });
      return windows;
    } catch (error) {
  ...
  async selectWindow(windowId) {
    ...
    const window2 = windows.find((w) => w.id === windowId);
    if (window2) {
      this.selectedWindow = window2;
      this.config.windowTitle = window2.title;
      this.saveConfig();
      console.log(`[DanmuCapture] Window selected: ${window2.title}`);
    }
    return window2 || null;
  }
  /**
  ...
  async captureWithOCR() {
    try {
      ...
      const targetSource = sources.find(
        (source) => {
          var _a;
          return source.name === ((_a = this.selectedWindow) == null ? void 0 : _a.title) || source.name.includes(this.config.windowTitle);
        }
      );
      if (!targetSource) {
   ...
   */
  generateSimulatedDanmu() {
    const sampleDanmu = [
      { username: "用户A", content: "主播真好看", type: "praise" },
      { username: "用户B", content: "支持下", type: "normal" },
      { username: "用户C", content: "关注了", type: "follower" }
    ];
    const count = Math.floor(Math.random() * 3);
    const selected = sampleDanmu.slice(0, count);
    return selected.map(
      (d) => this.createDanmu(d.username, d.content, d.type)
    );
  }
  /**
   * Create a Danmu object
   */
  createDanmu(username, content, type) {
    return {
   ...
   */
  calculateSentiment(type, content) {
    switch (type) {
      case "praise":
      case "gift":
      case "big_gift":
  ...
  deduplicateDanmu(danmuList) {
    ...
    for (const danmu of danmuList) {
      ...
      const isSimilar = this.dedupState.recentContents.some(
        (recent) => this.calculateSimilarity(danmu.content, recent) > this.dedupSimilarityThreshold
      );
      if (isSimilar) {
        continue;
      }
...
__publicField(_DanmuCaptureService, "instance", null);
let DanmuCaptureService = _DanmuCaptureService;
let mainWindow = null;
